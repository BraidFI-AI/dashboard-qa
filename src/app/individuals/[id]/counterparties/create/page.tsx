...
const CreateCounterpartyPage = () => {
  ...
  const onSubmit: SubmitHandler<CreateCounterparty> = (
    ...
    // } else if (association == "Business") {
    //   data.productId = undefined;
    //   data.individualId = undefined;
    //   data.accountId = undefined;
    // } else if (association == "Individual") {
    //   data.productId = undefined;
    //   data.businessId = undefined;
    //   data.accountId = undefined;
    // } else {
    ...
    try {
      ...
      data.individualId = parseInt((params.id as string) || "0");
    } catch (e) {
      enqueueSnackbar("Invalid Individual ID", { variant: "error" });
      return;
    }
    ...
    dispatch(createCounterparty(data)).then((data: any) => {
      if (typeof data.payload != "string") {
        ...
        router.replace(`/individuals/${params.id}/counterparties`);
      } else {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="pb-6">
      <Box className="flex flex-col w-1/3">
        ...
        />
        <Box className="pb-4"></Box>
        <ItemRow title="Individual ID" value={(params.id as string) || "0"} />
        {/* <RadioButton
          title="Association"
          value={association}
