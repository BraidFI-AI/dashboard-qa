...
export default function linkToCounterparty(counterparty: Counterparty | null) {
  ...
  if (counterparty.productId != null) {
    ...
    counterparty.accountNumber != null ||
    counterparty.accountId != null
  ) {
    return `/accounts/${
      counterparty.accountNumber ?? counterparty.accountId
    }/counterparties/${counterparty.id}`;
  } else if (counterparty.businessId != null) {
    return `/businesses/${counterparty.businessId}/counterparties/${counterparty.id}`;
  } else if (counterparty.individualId != null) {
    return `/individuals/${counterparty.individualId}/counterparties/${counterparty.id}`;
  } else {
    return null;
  }
}
